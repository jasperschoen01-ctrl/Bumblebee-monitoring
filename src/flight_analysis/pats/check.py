...
if __name__ == "__main__":
    ...
    # example_section = next(d for d in sections if d["id"] == 119)  # or, if you want a specific section and you already know the id
    ...
    today = datetime(2025,5,24,0,0,0)
    past_date = datetime(2024,5,24,0,0,0)
    ...
    if len(spots["c"]):
        ...
        some_insect_class: dict = next((item for item in example_section["detection_classes"] if item["id"]==64), {})

        # Initialize a start and end date, and set the time to 12:00:00:00 precisely.
        start_datetime = past_date.replace(hour=12, minute=0, second=0, microsecond=0)
        end_datetime = today.replace(hour=12, minute=0, second=0, microsecond=0)
        ...
        if df_detections.empty:
            ...
            # Save the detections data to a CSV file with unique name
            start_str = start_datetime.strftime('%Y%m%d')
            end_str = end_datetime.strftime('%Y%m%d')
            label = some_insect_class["label"]
            insect_label = label.replace(" ", "_")
            detections_filename = f"detections_{insect_label}_{start_str}_{end_str}.csv"
            df_detections.to_csv(detections_filename, index=False)
            print(f"Detections saved to {detections_filename}")

            # Download flight track for the first detection
            some_insect = df_detections["uid"].iloc[0]
            try:
                df_flight = pats_service.download_c_flight_track(example_section["id"], some_insect)
                # Save the flight track to a CSV with unique name
                flight_filename = f"flight_track_{insect_label}_{start_str}_{end_str}_{some_insect}.csv"
                df_flight.to_csv(flight_filename, index=False)
                print(f"Flight track saved to {flight_filename}")
                example_plots.c_flight_3d_plot(df_flight)
            except Exception as e:
                print(f"Failed to download flight track: {e}")

            # Download the video of the insect
            try:
                mkv_data = pats_service.download_c_video(example_section["id"], some_insect)
                video_filename = f"video_{insect_label}_{start_str}_{end_str}_{some_insect}.mkv"
